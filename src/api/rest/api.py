...
def register_api_routes(app, extensions, cache_control, config):
    ...
    @app.route("/api/ticks/recent", methods=["GET"])
    def get_recent_ticks():
        """
        Get recent tick data from ohlcv_1min table.

        Sprint 54: WebSocket Processing Simplification
        - Replaces WebSocket broadcast for frontend tick updates
        - Frontend polls this endpoint every 1-5 seconds
        - Returns OHLCV data from database persistence layer
        ...
        Returns:
            JSON array of OHLCV records: [{symbol, timestamp, open, high, low, close, volume}, ...]
        """
        try:
            ...
            if since_timestamp:
                ...
                since_dt = datetime.fromtimestamp(since_timestamp, tz=UTC)
                query = """
                    ...
                    LIMIT :limit
                """
                params = {"symbol": symbol.upper(), "since": since_dt, "limit": limit}
            else:
                # No timestamp filter - return most recent records
                query = """
                    SELECT symbol, timestamp, open, high, low, close, volume
                    FROM ohlcv_1min
                    WHERE symbol = :symbol
            ...
            # Execute query
            with tickstock_db.get_connection() as conn:
                ...
                ticks.append(
                    {
                        ...
                        if row[1]
                        else None,  # Convert datetime to Unix timestamp
            ...
            return jsonify({"symbol": symbol.upper(), "count": len(ticks), "ticks": ticks})

        except Exception as e:
            logger.error(f"Error fetching recent ticks for {symbol}: {e}")
    ...
    def api_health():
        ...
        try:
            ...
            try:
                ...
                if market_service:
                    ...
                    market_service_stats = {
                        ...
                        "data_adapter_connected": market_health.get(
                            "data_adapter_connected", False
                        ),
                    }
                else:
            ...
            return jsonify(
                {
                    ...
                    "stats": {"market_service": market_service_stats},
            ...
            return jsonify(
                {"status": "unhealthy", "error": str(e), "timestamp": datetime.now(UTC).isoformat()}
            ), 500
    ...
    def get_user_settings():
        ...
        try:
            ...
            return jsonify({"success": True, "settings": settings, "user_id": current_user.id})
        except Exception as e:
        ...
        try:
            ...
            if query:
                ...
                for symbol in symbols:
                    if query in symbol["symbol"] or query in symbol.get("name", "").upper():
                ...
                symbols = [{"symbol": s["symbol"], "name": s["name"]} for s in symbols]
        ...
        try:
            ...
            for symbol_code in watchlist_symbols:
                ...
                if symbol_details:
                    ...
                    watchlist_data.append(
                        {"symbol": symbol_code, "name": "Unknown", "last_price": 0.00}
                    )

            return jsonify({"success": True, "symbols": watchlist_data})
        ...
        try:
            ...
            if symbol not in watchlist:
                ...
                success = user_settings_service.set_user_setting(
                    current_user.id, "watchlist", watchlist
                )
    ...
    def api_remove_from_watchlist():
        ...
        try:
            ...
            if symbol in watchlist:
                ...
                return jsonify({"success": False, "error": "Failed to save watchlist"}), 500
            return jsonify({"success": True, "message": f"{symbol} was not in watchlist"})
    ...
    def api_get_chart_data(symbol):
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    "Using enhanced mock data for %s (%s): %d bars",
                    symbol,
                    timeframe,
                    len(chart_data),
                )
                ...
                return jsonify(
                    {"success": False, "error": f"Failed to generate chart data for {symbol}"}
                ), 500
    ...
    def update_pattern_subscription(pattern_name):
        ...
        try:
            ...
            return jsonify(
                {"success": True, "message": f"Pattern {pattern_name} updated successfully"}
            )
        except Exception as e:
    ...
    def unsubscribe_from_pattern(pattern_name):
        ...
        try:
            ...
            return jsonify({"success": True, "message": f"Unsubscribed from {pattern_name}"})
        except Exception as e:
    ...
    def submit_backtest():
        ...
        try:
            ...
            if not backtest_data:
                return jsonify({"success": False, "error": "No backtest data provided"}), 400
    ...
    def cancel_backtest_job(job_id):
        ...
        try:
            ...
            return jsonify({"success": True, "message": f"Backtest job {job_id} cancelled"})
        except Exception as e:
    ...
    def api_get_market_movers():
        ...
        try:
            ...
            logger.debug(
                "Market movers API: %d gainers, %d losers - %.2fms",
