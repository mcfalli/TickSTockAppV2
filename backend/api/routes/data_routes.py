...
from app_utils import generate_test_events
from database import User, db, Subscription
from backend.api.services.user_settings_service import UserSettingsService
from backend.config.logging_config import get_domain_logger, LogDomain

from backend.api.services.user_filters_service import UserFiltersService
from backend.utils.monitoring.tracer import tracer, TraceLevel, normalize_event_type, ensure_int
from backend.data_sources.websocket.websocket_publisher import WebSocketPublisher
...
def register_universe_api_routes(app, cache_control, user_settings_service, config):
    ...
    def select_universes():
        ...
        try:
            ...
            # 🔧 FIX: Get OLD state from database BEFORE any changes
    ...
    def get_user_universe_selections():
        """Get user's current universe selections from database"""
        try:
