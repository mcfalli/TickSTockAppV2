    ...
    def test_alerting_and_threshold_validation(self, health_monitoring_system):
        ...
        with patch.object(system['event_router'], 'route_event') as mock_slow_routing:
            ...
            with patch.object(universal_manager, '_find_interested_users') as mock_find:
                ...
                if 30.0 > test_thresholds['routing_time_critical_ms']:
                    scenario.add_alert(
                        'routing_critical',
                        f"Routing time 30.0ms exceeds critical threshold",
                        'critical'
                    )
                elif 30.0 > test_thresholds['routing_time_warning_ms']:
                    scenario.add_alert(
                        'routing_warning',
                        f"Routing time 30.0ms exceeds warning threshold",
                        'warning'
                    )
